...
class PNS(object):
    ...
    def automatic_fit_subspheres(self, data, dm, nullspdim, basisu=[]):
        ...
        for i in range(dm - 1):
            ...
            if is_isotropic:
                ...
                res_small = np.arccos(np.matmul(center_small.T, data)) - r_small

                center_great, r_great = self.get_subsphere(data)
                res_great = np.arccos(np.matmul(center_great.T, data)) - r_great
            ...
            res = np.arccos(np.matmul(center.T, data)) - r
    ...
    def get_subsphere(self, data, small_circle=False):
        ...
        def obj_fun(center, r, data):
            ...
            """
            return np.mean((np.arccos(np.matmul(center.T, data)) - r) ** 2)
